defmodule WatercoolerWeb.ChatLive do
  use WatercoolerWeb, :live_view
  on_mount {WatercoolerWeb.LiveUserAuth, :live_user_required}
  ...
  def mount(_params, _session, socket) do
    WatercoolerWeb.Endpoint.subscribe("chat:conversations:#{socket.assigns.current_user.id}")
    ...
    socket =
      ...
      |> stream(
        :conversations,
        Watercooler.Chat.my_conversations!(actor: socket.assigns.current_user)
      )
      |> assign(:messages, [])
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    conversation =
      Watercooler.Chat.get_conversation!(conversation_id, actor: socket.assigns.current_user)
    ...
    cond do
      ...
      socket.assigns[:conversation] ->
        WatercoolerWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
        WatercoolerWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")
    ...
    |> stream(:messages, Watercooler.Chat.message_history!(conversation.id, stream?: true))
    |> assign_message_form()
    |> then(&{:noreply, &1})
  end
  ...
  defp assign_message_form(socket) do
    form =
      if socket.assigns.conversation do
        Watercooler.Chat.form_to_create_message(
          actor: socket.assigns.current_user,
          private_arguments: %{conversation_id: socket.assigns.conversation.id}
        )
        |> to_form()
      else
        Watercooler.Chat.form_to_create_message(actor: socket.assigns.current_user)
        |> to_form()
      end
