defmodule Watercooler.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%Watercooler.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(Watercooler.Accounts.User, id, authorize?: false) do
